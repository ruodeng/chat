...
async function handleCreateRoom(request, ip, kv, turnstileSecret, turnstileSiteKey) {
  ...
  if (cbl) {
    ...
    return json({ error: `Too many creations, retry in ${remaining} minutes`, blacklisted: true, retryAfter: cbl.until - now }, 429);
  }
  ...
  if (recentCount >= 10) {
    ...
    return json({ error: 'Too many creations, blocked for 60 minutes', blacklisted: true, retryAfter: 3600_000 }, 429);
  }

  if (recentCount >= 3) {
    if (turnstileSiteKey) {
      if (!body.turnstile) {
        return json({ error: 'Too many creations, verification required', requireTurnstile: true }, 403);
      }
      const ok = await verifyTurnstile(turnstileSecret, body.turnstile, ip);
      if (!ok) {
        return json({ error: 'Verification failed, please retry', requireTurnstile: true }, 403);
      }
    } else {
      const cooldownEnd = tracker.timestamps[0] + 120_000;
      if (now < cooldownEnd) {
        const wait = Math.ceil((cooldownEnd - now) / 1000);
        return json({ error: `Too many creations, please wait ${wait} seconds`, retryAfter: cooldownEnd - now }, 429);
      }
    }
  }
...
async function handleJoinRoom(request, roomId, ip, kv, turnstileSecret, turnstileSiteKey) {
  ...
  if (bl) {
    ...
    return json({ error: `Too many attempts, blocked. Retry in ${remaining} minutes`, blacklisted: true, retryAfter: bl.until - now }, 429);
  }
  ...
  if (fails.count >= 3 && fails.count < 6) {
    ...
    if (elapsed < delay) {
      ...
      return json({ error: `Please wait ${wait} seconds`, retryAfter: delay - elapsed, fails: fails.count }, 429);
    }
  }
  ...
  if (turnstileSiteKey && fails.count >= 3) {
    if (!body.turnstile) {
      return json({ error: 'Too many wrong PINs, verification required', requireTurnstile: true, fails: fails.count }, 403);
    }
    const ok = await verifyTurnstile(turnstileSecret, body.turnstile, ip);
    if (!ok) {
      return json({ error: 'Verification failed, please retry', requireTurnstile: true, fails: fails.count }, 403);
    }
  }
  ...
  if (fails.count >= 6) {
    ...
    return json({ error: 'Too many attempts, blocked for 30 minutes', blacklisted: true, retryAfter: 30 * 60_000 }, 429);
  }
  ...
  return json({ error: msg, fails: fails.count, requireTurnstile: !!turnstileSiteKey && fails.count >= 3 }, 403);
}
...
let roomId=null,roomPin=null,roomTtl=1,myName='',lastTs=0,renderedIds=new Set(),pollingTimer=null,joinFailCount=0,turnstileEnabled=false,turnstileToken=null,turnstileWidgetId=null,cooldownTimer=null;
...
async function doCreateRoom(){try{const ttl=parseInt($('#ttl-select').value)||1;const body={ttl};if(turnstileToken){body.turnstile=turnstileToken;turnstileToken=null}const res=await fetch('/api/room/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await res.json();if(res.ok){resetJoinForm();roomTtl=data.ttlHours||1;joinAndEnter(data.roomId,data.pin);return}if(data.blacklisted){showJoinError(data.error);startCreateCooldown(data.retryAfter);return}if(data.retryAfter){showJoinError(data.error);startCreateCooldown(data.retryAfter);return}if(data.requireTurnstile){showJoinError(data.error);await maybeShowTurnstile();$('#btn-create').textContent=t('btnVerifyCreate');return}toast(data.error||t('toastCreateFail'))}catch{toast(t('toastNetError'))}}
...
async function joinAndEnter(id,pin){const body={pin};if(turnstileToken){body.turnstile=turnstileToken;turnstileToken=null}try{const res=await fetch('/api/room/'+id+'/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await res.json();if(res.ok){resetJoinForm();myName=d.name;try{const ir=await fetch('/api/room/'+id+'/info');const info=await ir.json();roomTtl=info.ttlHours||1}catch{roomTtl=1}enterRoom(id,pin);return}joinFailCount=d.fails||joinFailCount;if(d.blacklisted){showJoinError(d.error);startCooldown(d.retryAfter,t('btnJoin'));return}if(d.retryAfter&&!d.requireTurnstile){showJoinError(d.error);startCooldown(d.retryAfter,t('btnJoin'));return}if(d.requireTurnstile){showJoinError(d.error);await maybeShowTurnstile();$('#btn-join').textContent=t('btnVerifyJoin');return}showJoinError(d.error||t('toastPinError'));$('#join-pin').value='';$('#join-pin').focus()}catch{toast(t('toastNetError'))}}
function enterRoom(id,pin){roomId=id;roomPin=pin;sessionStorage.setItem('roomId',id);sessionStorage.setItem('roomPin',pin);history.replaceState(null,'','/room/'+id);landingView.style.display='none';chatView.classList.add('active');roomTag.textContent='#'+id;$('#ttl-info').textContent=roomTtl+'h '+t('ttlInfo');lastTs=0;renderedIds.clear();messagesEl.innerHTML='<div class="empty-hint"><span>'+t('emptyHint')+'</span><br><span style="font-size:11px">'+t('emptySub')+'</span></div>';fetchMessages();if(pollingTimer)clearInterval(pollingTimer);pollingTimer=setInterval(fetchMessages,2000)}
$('#btn-leave').addEventListener('click',()=>{if(pollingTimer)clearInterval(pollingTimer);sessionStorage.removeItem('roomId');sessionStorage.removeItem('roomPin');roomId=null;roomPin=null;myName='';history.replaceState(null,'','/');chatView.classList.remove('active');landingView.style.display=''});
...
async function sendFile(file){const fd=new FormData();fd.append('pin',roomPin);fd.append('sender',myName);fd.append('file',file);try{const res=await fetch('/api/room/'+roomId+'/send',{method:'POST',body:fd});if(!res.ok){toast(t('toastUploadFail'))}else{fetchMessages()}}catch{toast(t('toastNetError'))}}
